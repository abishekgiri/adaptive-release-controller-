...
*(scaffold — Abi to rewrite in own voice)*

We characterize the operating envelope of contextual-bandit deployment control: the set of cost-asymmetry, failure-rate, and trajectory-length regimes where adaptive policies outperform well-calibrated static rules. We apply disjoint LinUCB (Li et al., 2010) and Bayesian linear Thompson Sampling (Agrawal & Goyal, 2013) to the three-action deployment problem {deploy, canary, block} with an asymmetric cost matrix and a pending-reward buffer that enforces the delayed-feedback invariant.

The strongest single result is the cost-ratio sweep. The bandit advantage over a static rule scales monotonically with cost asymmetry: −7% at 5:1, −19% at 20:1, −49.6% at 100:1. The default 20:1 ratio sits at the envelope boundary where bandits break even with static rules; the case for adaptation strengthens materially only above 40:1. Within the bandit family, Thompson Sampling outperforms LinUCB by 6.8% on real CI data (Thompson 624 vs. LinUCB 669.5; bootstrap 95% CI [598, 648] entirely below LinUCB mean, p < 0.0001, 0/10,000 resamples). The same CI contains the static-rule cost (644.5), so Thompson and the static rule are statistically tied — the exploration-strategy choice matters more in this regime than the bandit-vs-rule choice.

The envelope has a hard lower boundary. On real GitHub Actions data (600 runs, two public repositories) with a 5.3% failure rate, LinUCB over-blocks and costs 3.8% more than a static rule; feature sparsity and a 300-step trajectory leave the policy below the O(d²) = 169 updates-per-arm convergence threshold. Drift adaptation using PageHinkley detection is net-negative across all three drift modes — stationary, abrupt, and gradual — due to false-alarm resets at the default sensitivity (λ = 50), triggering 10.6 resets per trajectory in a stationary environment. The bandit framing is most defensible as a thinking tool that makes cost assumptions explicit; algorithmic gains are conditional on operating above the envelope boundary.
...
The cleanest empirical evidence that the bandit framing pays off comes from the cost-ratio sweep (F11, §6.4): LinUCB's advantage over a static threshold rule grows monotonically from −7% at a 5:1 deploy-failure-to-block cost ratio to −49.6% at 100:1, with the default 20:1 ratio sitting precisely at the break-even boundary (+0.05%). The mechanism is cost-weighted gradient updates — when deploy failures are expensive relative to blocking, the negative-cost reward signal assigns large magnitudes to bad-action updates, steering the policy toward block-heavy strategies that a fixed-threshold rule cannot reach without manual recalibration. This operationalizes §1.3 Condition 1 directly: the bandit advantage is conditional on sufficient failure-cost asymmetry, and the cost-ratio sweep makes that condition continuous and measurable rather than binary. At ratios below 20:1, exploration cost exceeds the penalty gap and static rules are competitive; at 40:1 and above, the adaptation gain is large enough that even modest convergence periods produce material cost reduction. The operating envelope is not "always" but "when the cost ratio is steep enough to amortize exploration."

### 9.2 When Bandits Underperform

*(scaffold — Abi to rewrite in own voice)*

The three negative findings each illuminate a distinct failure mode. At low failure rates the deploy arm is nearly always optimal and exploration is wasted: on psf/requests (5.3% failure rate, F8), expected deploy cost is 0.53/step versus 1.92/step for block; a policy allocating 42.2% of decisions to block pays 3.8% more than a static rule that simply set a threshold appropriate for this failure-rate distribution. Short trajectories with sparse features prevent UCB convergence: the real-data evaluation uses 300 steps per project with d=13, yielding approximately 100 updates per arm — well below the O(d²)=169 convergence threshold from §1.3 Condition 2 — so confidence intervals never shrink to first-order accuracy and the UCB criterion cannot reliably differentiate arms on feature signal alone. Off-the-shelf drift detectors at default sensitivity fire on cost-stream variance rather than genuine concept shift: PageHinkley at λ=50 fires 10.6 times per 500-step stationary trajectory (F12), resetting learned weights on noise fluctuations and making the full drift-adaptive variant 8–28% more expensive across all three drift conditions tested. All three failure modes are predictable from the framework conditions in §1.3 — they are not surprises but illustrations of where the envelope edges are. Well-calibrated static rules remain a defensible baseline at default 20:1 asymmetry on short trajectories with sparse features.

### 9.3 Practical Deployment Considerations

*(scaffold — Abi to rewrite in own voice)*

An industry team adopting this framing would face three calibration problems this paper does not resolve. The cost matrix (deploy_failure, block_bad) must be calibrated per project or service tier — a 5:1 ratio appropriate for an internal tool is structurally different from a 100:1 ratio for a payment-critical service, and the paper's 20:1 default sits at the break-even boundary and should not be treated as a safe generic choice. The drift-detector threshold λ_PH must be estimated from the project's observed cost-stream variance rather than set to a package default; the 10.6 false alarms per trajectory at λ=50 on stationary data are a calibration artifact, not an inherent cost of the approach, and a project-specific sweep over λ would be prerequisite before enabling drift-triggered resets. Finally, the policy should be bootstrapped on a logged-action dataset before going live to avoid cold-start over-blocking during the convergence period; deploying an untrained policy in a low-failure-rate environment risks exactly the psf/requests failure mode documented in §5.4. The bandit framing is most defensible as a thinking tool — making cost asymmetry explicit, surfacing the canary action as a first-class decision, framing deployment sequentially rather than as a binary classification — and the algorithmic gains are conditional on the operating regime.
