...
\tightlist
\item
  \textbf{The threshold encodes an implicit cost assumption} --- a
  judgment about the cost of blocking safe changes vs.~deploying bad
  ones that is rarely explicit and never adapts.
\item
  ...
  one at an internal tool, which a single fixed threshold cannot
  represent.
\item
  \textbf{The model does not learn from outcomes:} JIT models are
  trained offline on defect labels and receive no feedback when
...
\item
  ...
  maximization} --- evaluated on what it costs to act, not on whether a
  label is predicted correctly.
\item
  \textbf{The three-action space makes the canary a first-class
  decision,} modeled directly rather than via a threshold on a risk
  score.
\item
  \textbf{The policy learns online from its own decisions:} each outcome
  updates its belief about which actions are cost-effective in which
  contexts.
\end{itemize}
